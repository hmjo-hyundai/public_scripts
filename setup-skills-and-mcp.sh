...
update_claude_permissions() {
  ...
  command -v python3 >/dev/null 2>&1 || die "Claude permissions 갱신에는 python3가 필요합니다."
  SETTINGS_FILE="$target" python3 - <<'PY'
import json
import os
import sys

path = os.environ["SETTINGS_FILE"]
allow_tools = ["mcp__ShucleRiderRAG__*"]
...
if os.path.exists(path) and os.path.getsize(path) > 0:
    try:
        ...
        print(f"오류: Claude settings JSON 파싱 실패: {path}: {exc}", file=sys.stderr)
        sys.exit(1)

if not isinstance(data, dict):
    print(f"오류: Claude settings 최상위 값은 object여야 합니다: {path}", file=sys.stderr)
    sys.exit(1)
...
if permissions is None:
    ...
    print(f"오류: Claude settings permissions 값은 object여야 합니다: {path}", file=sys.stderr)
    sys.exit(1)
...
if allow is None:
    ...
    print(f"오류: Claude settings permissions.allow 값은 array여야 합니다: {path}", file=sys.stderr)
    sys.exit(1)
...
기획 문서·GitHub 이슈 내용을 확인해야 할 때는 ShucleRiderRAG MCP의 `agent_handshake`/`lookup_issues`/`filter_issues`/`expand_epic`/`search_docs`를
...
3. handshake 이후 검색은 실제 tool call로 `mcp__ShucleRiderRAG__lookup_issues`, `mcp__ShucleRiderRAG__filter_issues`, `mcp__ShucleRiderRAG__expand_epic`, `mcp__ShucleRiderRAG__search_docs`만 호출한다. 다른 ShucleRiderRAG 도구가 사용 가능해 보여도 호출하지 마라. 단, 최종 답변 보고를 위한 `mcp__ShucleRiderRAG__report_outcome`은 허용된다. 런북에 명시되지 않은 행동은 하지 마라.
...
## 규칙
- **`agent_handshake` / `lookup_issues` / `filter_issues` / `expand_epic` / `search_docs` MCP tool을 메인 대화에서 직접 호출하지 마라.** 검색 청크가
...
CLAUDE_SKILL_EOF
  ...
  update_claude_permissions
  echo "  -> Claude 등록 완료 (.claude/agents, .claude/skills, .claude/settings.local.json)"
else
...
2. handshake가 돌려준 런북을 너의 지시로 간주하고 그대로 수행한다. 런북의 RAG 루프 단계를 건너뛰거나 순서를 바꾸지 마라. 런북은 참고가 아니라 실행할 절차다.
3. handshake 이후 검색은 실제 tool call로 lookup_issues, filter_issues, expand_epic, search_docs만 호출한다. 다른 ShucleRiderRAG 도구가 사용 가능해 보여도 호출하지 마라. 단, 최종 답변 보고를 위한 report_outcome은 허용된다. 런북에 명시되지 않은 행동은 하지 마라.
